...
// 简单的3D field写入CSV函数（按z切片）
void field3_to_csv_slices(const field3& f, const std::string& base_filename)
{
    int nx = f.get_nx();
    int ny = f.get_ny();
    int nz = f.get_nz();
    ...
    for (int k = 0; k < nz; k++)
    {
        std::string filename = base_filename + "_z" + std::to_string(k) + ".csv";
...
int main(int argc, char* argv[])
{
    ...
    // 输出结果（按z切片）
    field3_to_csv_slices(v_T1, "result/v_T1");
    field3_to_csv_slices(v_T2, "result/v_T2");
    field3_to_csv_slices(v_T3, "result/v_T3");
    field3_to_csv_slices(v_T4, "result/v_T4");

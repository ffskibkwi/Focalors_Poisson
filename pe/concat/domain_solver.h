...
public:
    ...
    virtual void solve_collective_root_owned(field2& f, bool is_debugmode = true) { solve(f, is_debugmode); }
...
public:
    ...
    // 集体求解（约定：通信器 root 持有完整 b，其他进程仅参与通信）
    virtual void solve_collective_root_owned(field3& f) { solve(f); }

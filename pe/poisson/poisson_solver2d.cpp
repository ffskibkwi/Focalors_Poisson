...
PoissonSolver2D::PoissonSolver2D(int in_nx, int in_ny, double in_hx, double in_hy, PDEBoundaryType in_boundary_type_left, PDEBoundaryType in_boundary_type_right, PDEBoundaryType in_boundary_type_down, PDEBoundaryType in_boundary_type_up, EnvironmentConfig* in_env_config)
    : nx(in_nx), 
    ny(in_ny), 
    hx(in_hx), 
    hy(in_hy), 
...
void PoissonSolver2D::cal_lambda()  //The current version is only for OpenMP
{
    ...
    for (int i = 1; i <= ny; i++)
    {
        if (boundary_type_down == PDEBoundaryType::Periodic &&
        boundary_type_up == PDEBoundaryType::Periodic) // P-P
        {
        lambda_y[i - 1] = -2.0 + 2.0 * std::cos(2.0 * pi / ny * std::floor(i / 2.0));
        }
        else if (boundary_type_down == PDEBoundaryType::Neumann &&
            boundary_type_up == PDEBoundaryType::Neumann) // N-N
        {
        lambda_y[i - 1] = -2.0 + 2.0 * std::cos(pi / ny * i);
            ...
            (boundary_type_up == PDEBoundaryType::Dirichlet || boundary_type_up == PDEBoundaryType::Adjacented)) // D/Adj - D/Adj
        {
        lambda_y[i - 1] = -2.0 + 2.0 * std::cos(pi / (ny + 1) * i);
        }
        else if (((boundary_type_down == PDEBoundaryType::Dirichlet || boundary_type_down == PDEBoundaryType::Adjacented) && boundary_type_up == PDEBoundaryType::Neumann) ||
            (boundary_type_down == PDEBoundaryType::Neumann && (boundary_type_up == PDEBoundaryType::Dirichlet || boundary_type_up == PDEBoundaryType::Adjacented))) // (D/Adj)-N or N-(D/Adj)
        {
...
void PoissonSolver2D::boundary_assembly()
{

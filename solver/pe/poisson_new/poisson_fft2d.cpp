...
// === Dirichlet-Dirichlet ===
PoissonFFT2D_DD::~PoissonFFT2D_DD()
{
    if (plan) fftw_destroy_plan(plan);
    if (ex_vec)
    {
        for (int i = 0; i < nx; ++i) delete[] ex_vec[i];
        delete[] ex_vec;
    }
    if (fft_result)
    {
        for (int i = 0; i < nx; ++i) delete[] fft_result[i];
        delete[] fft_result;
    }
}

void PoissonFFT2D_DD::init(int in_nx, int in_ny)
{
    nx = in_nx; ny = in_ny;
    ...
    for (int i = 0; i < nx; i++)
    {
        ...
        for (int j = 0; j < 2 * ny + 2; j++) ex_vec[i][j] = 0.;
    }

    fft_result = new fftw_complex*[nx];
    for (int i = 0; i < nx; i++) fft_result[i] = new fftw_complex[ny + 2];

    plan = fftw_plan_dft_r2c_1d(2 * ny + 2, ex_vec[0], fft_result[0], FFTW_MEASURE);
}

void PoissonFFT2D_DD::transform(const field2& f, field2& f_hat)
{
    OPENMP_PARALLEL_FOR()
    for (int i = 0; i < nx; i++)
    {
...
void PoissonFFT2D_DD::transform_transpose(const field2& p_hat, field2& p)
{
    OPENMP_PARALLEL_FOR()
    for (int i = 0; i < nx; i++)
    {
        for (int j = 0; j < ny; j++) ex_vec[i][j + 1] = p_hat(i, j);
        fftw_execute_dft_r2c(plan, ex_vec[i], fft_result[i]);
        for (int j = 0; j < ny; j++) p(i, j) = -sqr_2_N1 * fft_result[i][j + 1][1];
    }
}

// === Neumann-Neumann ===
PoissonFFT2D_NN::~PoissonFFT2D_NN()
{
    if (plan) fftw_destroy_plan(plan);
    delete[] cos_ipi_n;
    delete[] sin_ipi_n;
...
void PoissonFFT2D_NN::init(int in_nx, int in_ny)
{
    nx = in_nx; ny = in_ny;
    ...
    for (int i = 0; i < nx; i++)
    {
        ...
        for (int j = 0; j < 2 * ny; j++) ex_vec[i][j] = 0.;
    }

    fft_result = new fftw_complex*[nx];
    for (int i = 0; i < nx; i++) fft_result[i] = new fftw_complex[ny + 1];

    plan = fftw_plan_dft_r2c_1d(2 * ny, ex_vec[0], fft_result[0], FFTW_MEASURE);
}

void PoissonFFT2D_NN::transform(const field2& f, field2& f_hat)
{
    OPENMP_PARALLEL_FOR()
    for (int i = 0; i < nx; i++)
    {
        for (int j = 0; j < ny; j++) ex_vec[i][j + 1] = (j % 2 == 0 ? -1 : 1) * f(i, j);
        fftw_execute_dft_r2c(plan, ex_vec[i], fft_result[i]);
        for (int j = 0; j < (ny - 1); j++)
            f_hat(i, j) =
                sqr_2_n * (-1. * cos_ipi_n[j] * fft_result[i][j + 1][1] - sin_ipi_n[j] * fft_result[i][j + 1][0]);
        f_hat(i, ny - 1) = 0.;
        for (int j = 0; j < ny; j++) f_hat(i, ny - 1) += sqr_n * f(i, j);
    }
}

void PoissonFFT2D_NN::transform_transpose(const field2& p_hat, field2& p)
{
    OPENMP_PARALLEL_FOR()
    for (int i = 0; i < nx; i++)
    {
        for (int j = 0; j < ny; j++) ex_vec[i][j + 1] = cos_ipi_n[j] * p_hat(i, j);
        fftw_execute_dft_r2c(plan, ex_vec[i], fft_result[i]);
        for (int j = 0; j < ny; j++) p(i, j) = (j % 2 == 0 ? 1 : -1) * sqr_2_n * fft_result[i][j + 1][1];
        for (int j = 0; j < ny; j++) ex_vec[i][j + 1] = sin_ipi_n[j] * p_hat(i, j);
        fftw_execute_dft_r2c(plan, ex_vec[i], fft_result[i]);
        for (int j = 0; j < ny; j++) p(i, j) += (j % 2 == 0 ? 1 : -1) * sqr_2_n * fft_result[i][j + 1][0];
        for (int j = 0; j < ny; j++) p(i, j) += (sqr_2_n + sqr_n) * p_hat(i, ny - 1);
    }
}

// === Neumann-Dirichlet ===
PoissonFFT2D_ND::~PoissonFFT2D_ND()
{
    if (plan) fftw_destroy_plan(plan);
...
void PoissonFFT2D_ND::init(int in_nx, int in_ny)
{
    nx = in_nx; ny = in_ny;
    ...
    for (int i = 0; i < nx; i++)
    {
        ...
        for (int j = 0; j < 2 * ny + 1; j++) ex_vec[i][j] = 0.;
    }
...
void PoissonFFT2D_ND::transform(const field2& f, field2& f_hat)
{
    OPENMP_PARALLEL_FOR()
    for (int i = 0; i < nx; i++)
    {
        for (int j = 0; j < ny; j++) ex_vec[i][j + 1] = (j % 2 == 0 ? -1 : 1) * sqr_4_2np1 * f(i, j);
        fftw_execute_dft_r2c(plan, ex_vec[i], fft_result[i]);
        for (int j = 0; j < ny; j++)
...
void PoissonFFT2D_ND::transform_transpose(const field2& p_hat, field2& p)
{
    OPENMP_PARALLEL_FOR()
    for (int i = 0; i < nx; i++)
    {
        for (int j = 0; j < ny; j++) ex_vec[i][j + 1] = sqr_4_2np1 * sin_2nipi_2np1[j] * p_hat(i, j);
        fftw_execute_dft_r2c(plan, ex_vec[i], fft_result[i]);
        for (int j = 0; j < ny; j++) p(i, j) = fft_result[i][j + 1][0];
        for (int j = 0; j < ny; j++) ex_vec[i][j + 1] = sqr_4_2np1 * cos_2nipi_2np1[j] * p_hat(i, j);
        fftw_execute_dft_r2c(plan, ex_vec[i], fft_result[i]);
        for (int j = 0; j < ny; j++) p(i, j) -= fft_result[i][j + 1][1];
        for (int j = 0; j < ny; j++) p(i, j) = (j % 2 == 0 ? -1 : 1) * p(i, j);
    }
}

// === Dirichlet-Neumann ===
PoissonFFT2D_DN::~PoissonFFT2D_DN()
{
    if (plan) fftw_destroy_plan(plan);
    if (ex_vec)
    {
...
void PoissonFFT2D_DN::init(int in_nx, int in_ny)
{
    nx = in_nx; ny = in_ny;
...
void PoissonFFT2D_DN::transform(const field2& f, field2& f_hat)
{
    OPENMP_PARALLEL_FOR()
    for (int i = 0; i < nx; i++)
    {
        for (int j = 0; j < ny; j++) ex_vec[i][j + 1] = (j % 2 == 0 ? -1 : 1) * sqr_4_2np1 * f(i, j);
        fftw_execute_dft_r2c(plan, ex_vec[i], fft_result[i]);
        for (int j = 0; j < ny; j++) f_hat(i, j) = (j % 2 == 0 ? 1 : -1) * fft_result[i][j + 1][1];
    }
}

void PoissonFFT2D_DN::transform_transpose(const field2& p_hat, field2& p)
{
    OPENMP_PARALLEL_FOR()
    for (int i = 0; i < nx; i++)
    {
        for (int j = 0; j < ny; j++) ex_vec[i][j + 1] = (j % 2 == 0 ? -1 : 1) * sqr_4_2np1 * p_hat(i, j);
        fftw_execute_dft_r2c(plan, ex_vec[i], fft_result[i]);
        for (int j = 0; j < ny; j++) p(i, j) = (j % 2 == 0 ? 1 : -1) * fft_result[i][j + 1][1];
    }
}

// === Periodic-Periodic ===
PoissonFFT2D_PP::~PoissonFFT2D_PP()
{
    delete[] cos_ipi_ny;
    delete[] sin_ipi_ny;

    if (plan_y_cos_odd) fftw_destroy_plan(plan_y_cos_odd);
    if (plan_y_sin_odd) fftw_destroy_plan(plan_y_sin_odd);
    if (plan_y_cos_even) fftw_destroy_plan(plan_y_cos_even);
    if (plan_y_sin_even) fftw_destroy_plan(plan_y_sin_even);
    if (plan_y_cos) fftw_destroy_plan(plan_y_cos);
    if (plan_y_sin) fftw_destroy_plan(plan_y_sin);

    if (ex_vector_y_sin_even)
    {
        for (int i = 0; i < nx; i++)
        {
...
void PoissonFFT2D_PP::init(int in_nx, int in_ny)
{
    nx = in_nx; ny = in_ny;
...
void PoissonFFT2D_PP::transform(const field2& f, field2& f_hat)
{
    OPENMP_PARALLEL_FOR()
    for (int i = 0; i < nx; i++)
    {
        for (int j = 0; j < ny - 1; j++) ex_vector_y_sin[i][j + 1] = f(i, j);
...
void PoissonFFT2D_PP::transform_transpose(const field2& p_hat, field2& p)
{
    OPENMP_PARALLEL_FOR()
    for (int i = 0; i < nx; i++)
    {
        ...
        for (int j = 0; j < ny; j++) p(i, j) = sqr_2_ny * (fc_y[i][j] + fs_y[i][j]);
    }
}